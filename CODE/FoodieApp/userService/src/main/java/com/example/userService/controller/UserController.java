...
public class UserController {
    ...
    public ResponseEntity<?> addItem(HttpServletRequest request,@RequestBody Favourites favourites){
        String email = (String)request.getAttribute("attr1");
        if(email.isEmpty()){
            return new ResponseEntity<>("No value is Present for email",HttpStatus.BAD_REQUEST);
        }
        else
            return new ResponseEntity<>(iUserService.addFavouritesInList(email, favourites),HttpStatus.OK);
    }
